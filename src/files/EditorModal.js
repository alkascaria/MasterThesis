    ...
    >
      ...
      <div style={centeredContentStyles} className="modal-content">
        ...
        <div style={inputStyles}>
          <label htmlFor="newGroupId" style={{ marginRight: "10px" }}>
            Enter new experiment group:
          </label>
          <input
            id="newGroupId"
            type="text"
            ...
            onChange={(e) => {
              ...
              if (exists) {
                setWarningMessage(
                  "Group ID already exists. Please enter a new one."
                );
              } else {
                setWarningMessage("");
              }
        ...
        <div style={inputStyles}>
          <label htmlFor="docName" style={{ marginRight: "10px" }}>
        ...
        <button
          onClick={() => {
            if (selectedOption && newGroupId) {
              setWarningMessage(
                "Please select ONE, either an existing group or enter a new group"
              );
            } else if (selectedOption || newGroupId) {
              let groupToSave = selectedOption ? selectedOption : newGroupId;
              let setGroupFunction = selectedOption
                ? setSelectedOption
                : setNewGroupId;
              saveToDB(
                groupToSave,
              ...
              setWarningMessage(
                "Please either select an existing group or enter a new group"
              );
            }
          }}
        >
